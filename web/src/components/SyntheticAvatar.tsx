 ...
 * Mirror split layout. Renders a hand-tuned face mesh of ~390 points
 ...
 *
 * When a `sentenceId` is provided the component loads the matching
 * pre-rendered landmark JSON from `/demo/avatar/{sentenceId}.json`
 * (generated by `npm run generate-avatar-frames`) and plays the
 * stored mouth_open_series synchronized with the stage clock. This
 * is the v02 §6.7 + §14.3 "pre-recorded landmark sequence" path;
 * dropping a real Mandarin-native MediaPipe recording into the same
 * JSON schema upgrades the avatar without code changes.
 *
 * If the JSON isn't reachable (offline, fetch error) the component
 * silently falls back to a fully procedural speech envelope — the
 * same one used in earlier builds — so the stage never freezes.
 */
interface Props {
  /** When true, animate the mouth area; otherwise hold the resting face. */
  speaking?: boolean;
  /** Sentence id — used to fetch the pre-rendered landmark JSON. */
  sentenceId?: string;
  className?: string;
}
...
interface RecordedTrack {
  fps: number;
  mouth_open_series: number[];
}

export function SyntheticAvatar({ speaking = true, sentenceId, className }: Props) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const pointsRef = useRef<Point[] | null>(null);
  const neighboursRef = useRef<number[][] | null>(null);
  const trackRef = useRef<RecordedTrack | null>(null);

  // Load the per-sentence pre-rendered track from /demo/avatar/ when
  // sentenceId is known. Silently falls back to the procedural
  // envelope if the JSON isn't reachable.
  useEffect(() => {
    if (!sentenceId) {
    ...
    let cancelled = false;
    fetch(`/demo/avatar/${sentenceId}.json`)
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => {
        if (cancelled || !data) return;
        if (
          typeof data.fps === "number" &&
          Array.isArray(data.mouth_open_series)
        ) {
          trackRef.current = {
            fps: data.fps,
            mouth_open_series: data.mouth_open_series,
          };
        }
      })
      .catch(() => {
        // Network error / 404 — fall back to procedural envelope.
        trackRef.current = null;
      });
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      // Prefer the loaded recording track when present; loop it so
      // longer Mirror sessions still see the mouth animate. Otherwise
      // run the procedural two-sine envelope.
      let mouthOpen: number;
      const track = trackRef.current;
      if (track && speaking && track.mouth_open_series.length > 0) {
        const frameIdx =
          Math.floor(t * track.fps) % track.mouth_open_series.length;
        mouthOpen = track.mouth_open_series[frameIdx];
      } else {
        const env = speaking
          ? 0.45 + 0.3 * Math.sin(t * 4.2) + 0.18 * Math.sin(t * 9.5 + 1)
          : 0.05;
        mouthOpen = Math.max(0.05, Math.min(0.95, env));
      }
